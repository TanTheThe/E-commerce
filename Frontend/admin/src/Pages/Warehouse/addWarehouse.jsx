...
const AddWarehouseModal = ({ open, onClose, onWarehouseCreated, context }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await postDataApi('/admin/warehouse/', submitData);

            if (response.success) {
                context.openAlertBox('success', 'Tạo kho thành công');
                onWarehouseCreated();
                handleClose();
            } else {
                context.openAlertBox('error', response?.data?.detail?.message || 'Tạo kho thất bại');
            }
        } catch (error) {
            console.error('Error creating warehouse:', error);
            context.openAlertBox('error', 'Lỗi hệ thống khi tạo kho');
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4"
            ...
            <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
                    <h2 className="text-xl font-bold text-gray-800">Tạo kho mới</h2>
                    <button
                        onClick={handleClose}
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2 pt-2">
                            <label className="flex items-center gap-2 cursor-pointer">
                                ...
                                />
                                <span className="text-sm text-gray-700">Kích hoạt kho ngay</span>
                            </label>
                        </div>
                    </div>
