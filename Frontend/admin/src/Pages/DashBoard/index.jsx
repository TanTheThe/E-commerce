...
import { FaAngleDown, FaAngleUp, FaLock, FaPlus, FaRegEye } from "react-icons/fa";
...
const DashBoard = () => {
    ...
    const LockedOverlay = ({ message = "Bạn không có quyền xem thông tin này" }) => (
        <div className="absolute inset-0 bg-white/95 backdrop-blur-sm flex flex-col items-center justify-center z-10 rounded-md">
            <FaLock className="text-gray-400 text-5xl mb-4" />
            <p className="text-gray-600 font-medium text-lg">{message}</p>
            <p className="text-gray-400 text-sm mt-2">Chỉ Admin mới có quyền truy cập</p>
        </div>
    );
        ...
        <>
            <div className="w-full py-2 p-5 border bg-[#f1faff] border-[rgba(0,0,0,0.1)] flex items-center gap-8 mb-5 justify-between rounded-md">
                <div className="info">
                    ...
                    <Button
                        className={`btn-blue !capitalize ${isStaff ? 'opacity-50 cursor-not-allowed' : ''}`}
                        onClick={() => {
                            if (!isStaff) {
            ...
            <div className="relative">
                {isStaff && <LockedOverlay message="Thông tin thống kê bị khóa" />}
                <div className={isStaff ? 'filter blur-sm pointer-events-none select-none' : ''}>
                    <DashBoardBoxes />
                </div>
            </div>

            <div className="pt-5 relative">
                {isStaff && <LockedOverlay message="Danh sách sản phẩm bị khóa" />}
                <div className={isStaff ? 'filter blur-sm pointer-events-none select-none' : ''}>
                    <Products />
                </div>
            </div>

            <div className="card my-4 shadow-md sm:rounded-lg bg-white relative">
                {isStaff && <LockedOverlay message="Thông tin đơn hàng bị khóa" />}
                <div className={isStaff ? 'filter blur-sm pointer-events-none select-none' : ''}>
                    <Orders />
                </div>
            </div>

            <div className="card my-4 shadow-md sm:rounded-lg bg-white relative overflow-x-hidden">
                {isStaff && <LockedOverlay message="Biểu đồ phân tích bị khóa" />}
                <div className={isStaff ? 'filter blur-sm pointer-events-none select-none' : ''}>
