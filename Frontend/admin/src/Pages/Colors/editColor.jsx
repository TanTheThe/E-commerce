...
const EditColorModal = ({ open, onClose, onColorUpdated, context, colorToEdit }) => {
    ...
    useEffect(() => {
        if (colorToEdit && open) {
            ...
            const colorCode = colorToEdit.code || '#ff0000';
            setSelectedColor(colorCode);

            const [h, s, v] = hexToHsv(colorCode);
            setHue(h);
            setSaturation(s);
            setBrightness(v);
        }
    ...
    const handleSubmit = async (e) => {
        ...
        if (!colorToEdit?.id) {
            context.openAlertBox('error', 'Không tìm thấy thông tin màu cần cập nhật');
            return;
        }

        setLoading(true);
        try {
            const response = await putDataApi(`/admin/color/${colorToEdit.id}`, {
                name: colorName.trim(),
                code: selectedColor.toUpperCase()
            });

            if (response.success) {
                context.openAlertBox('success', response.message || 'Cập nhật màu thành công!');
                onColorUpdated();
                handleClose();
            } else {
                context.openAlertBox('error', response.message || 'Có lỗi xảy ra khi cập nhật màu');
            }
        } catch (error) {
            console.error('Error updating color:', error);
            context.openAlertBox('error', 'Lỗi hệ thống khi cập nhật màu');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        ...
        >
            ...
            <DialogActions sx={{
                ...
                >
                    {loading ? 'Đang cập nhật...' : 'Cập nhật màu'}
                </Button>
            </DialogActions>
        </Dialog>
    );
