...
class ToggleWarehouseStatusService:
    async def toggle_warehouse_status(self, warehouse_id: str, session: AsyncSession):
        ...
        if warehouse.is_active and warehouse.is_default:
            WareHouseException.inactive_must_be_not_default()

        new_status = not warehouse.is_active

        await warehouse_repository.update_warehouse(condition, {"is_active": new_status, "updated_at": datetime.now()}, session)
