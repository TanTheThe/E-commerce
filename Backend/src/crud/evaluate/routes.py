...
from src.schemas.evaluate import EvaluateInputModel, SupplementEvaluateModel, GetEvaluateByProduct
...
@evaluate_customer_router.get("/my-reviews", status_code=status.HTTP_200_OK, dependencies=[Depends(customer_role_middleware)])
...
async def get_all_evaluate_admin(skip: int = 0, limit: int = 10,
                                 token_details: dict = Depends(access_token_bearer),
                                 session: AsyncSession = Depends(get_session)):
