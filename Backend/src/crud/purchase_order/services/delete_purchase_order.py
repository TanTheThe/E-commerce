...
class DeletePurchaseOrderService:
    async def delete_purchase_order(self, po_id: str, session: AsyncSession):
        ...
        has_receipts = await goods_receipt_repository.get_goods_receipt(session=session,
                                                                        where_conditions=condition_receipts)
        if has_receipts:
            PurchaseOrderException.cant_delete_po_has_goods_receipts()

        condition_payment = [SupplierPayment.purchase_order_id == po_id]
        has_payments = await supplier_repository.get_supplier_payment(session=session,
                                                                      where_conditions=condition_payment)
        if has_payments:
