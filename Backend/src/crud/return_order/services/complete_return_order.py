...
class CompleteReturnOrderService:
    async def complete_return(self, return_order_id: str, restore_stock: bool, request: Request, session: AsyncSession):
        ...
        if return_order.order.payment_method == "vnpay":
            refund_result = await self.process_refund_single_attempt(return_order, request, session)
    ...
    async def process_refund_single_attempt(self, return_order: ReturnOrder, request: Request, session: AsyncSession):
