...
from src.schemas.stock import ProductStockStatus, SortBy, SortOrder, ProductsSummaryQueryParams
...
class GetProductsSummaryService:
    MAX_LIMIT = 100
    ...
    async def get_products_summary(self, session: AsyncSession, warehouse_id: str,
                                   params: ProductsSummaryQueryParams,
        ...
        where_conds = self.build_where_conditions(
            params.search,
            params.category_ids,
            params.brand_ids,
            params.stock_status,
        ...
        if params.sort_by == SortBy.NAME:
            order_col = Product.name
        elif params.sort_by == SortBy.TOTAL_QUANTITY:
            order_col = stock_summary.c.total_quantity
        else:
            order_col = stock_summary.c.last_inbound_date

        order_by_clause = order_col.desc() if params.sort_order == SortOrder.DESC else order_col.asc()
        ...
        products_data = [
            self.build_product_response(row, categories_map.get(str(row.product_id), []))
            for row in results
        ]
    ...
    def build_product_response(self, row, categories: List[dict]) -> dict:
