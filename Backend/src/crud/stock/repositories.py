...
class StockRepository:
    ...
    async def get_stock_transactions(self, conditions: List[Optional[ColumnElement[bool]]], session: AsyncSession,
                                     skip: int = 0, limit: int = 10,
                                     joins: list = None, order_by_clause=None):
        count_stmt = select(func.count(StockTransaction.id)).where(*conditions)
        total_result = await session.exec(count_stmt)
        total = total_result.one()

        statement = select(StockTransaction).where(*conditions).options(
            *joins if joins else []
    ...
    async def get_all_stocks(self, conditions: List[Optional[ColumnElement[bool]]], session: AsyncSession,
                                  skip: int = 0, limit: int = 10,
                                  joins: list = None, order_by_clause=None):
        count_stmt = select(func.count(Stock.id)).where(*conditions)
        total_result = await session.exec(count_stmt)
        total = total_result.one()

        statement = select(Stock).where(*conditions).options(
            *joins if joins else []
        ).offset(skip).limit(limit)
