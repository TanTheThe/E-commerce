from datetime import datetime
from typing import Dict, List
from sqlalchemy.orm import selectinload
from sqlmodel.ext.asyncio.session import AsyncSession
from src.cache import cache_service
from src.crud.good_receipts.repositories import GoodsReceiptRepository
from src.crud.good_receipts.services.approve_goods_receipt.approval_calculation import ApprovalCalculationService
from src.crud.good_receipts.services.approve_goods_receipt.approval_validation import ApproveGRValidationService
from src.crud.good_receipts.services.get_approval_preview.receipt_tree import ReceiptTreeService
from src.crud.stock.services.update_stock_completed_receipts import StockUpdateCompletedReceiptService
from src.database.models import GoodsReceipt, PurchaseOrder
import logging

logger = logging.getLogger(__name__)
goods_receipt_repository = GoodsReceiptRepository()
validation_service = ApproveGRValidationService()
receipt_tree_service = ReceiptTreeService()
calculation_service = ApprovalCalculationService()
stock_update_service = StockUpdateCompletedReceiptService()


class ApproveGoodsReceiptService:
    async def approve_goods_receipt(self, goods_receipt_id: str, approved_by: str, session: AsyncSession) -> Dict:
        conditions = [GoodsReceipt.id == goods_receipt_id]

        options = [
            selectinload(GoodsReceipt.purchase_order).selectinload(PurchaseOrder.po_details),
            selectinload(GoodsReceipt.warehouse),
            selectinload(GoodsReceipt.supplier),
            selectinload(GoodsReceipt.receipt_details)
        ]

        gr = await goods_receipt_repository.get_goods_receipt(
            session=session,
            where_conditions=conditions,
            options=options
        )

        self.validate_goods_receipt(gr)

        po = gr.purchase_order

        all_related_grs = await receipt_tree_service.get_all_related_receipts(gr, str(po.id), session)

        variant_summary = calculation_service.calculate_variant_summary(all_related_grs, gr)

        po_details_map = {str(detail.id): detail for detail in po.po_details}

        status_info = calculation_service.determine_completion_status(variant_summary, po_details_map)

        await self.mark_gr_as_approved(gr, approved_by, status_info['gr_status'])

        if status_info['all_completed']:
            result = await self.handle_completed_case(
                gr, po, all_related_grs, approved_by, session, status_info
            )
        else:
            result = await self.handle_partial_case(
                gr, po, po_details_map, session, status_info
            )

        await session.commit()

        await session.refresh(gr)

        return result



    def validate_goods_receipt(self, gr):
        validation_service.validate_gr_exists(gr)
        validation_service.validate_gr_status_pending(gr)
        validation_service.validate_po_exists(gr)
        validation_service.validate_warehouse_active(gr)
        validation_service.validate_supplier_active(gr)
        validation_service.validate_has_receipt_details(gr)
        validation_service.validate_all_accepted_positive(gr)

    async def mark_gr_as_approved(self, gr, approved_by: str, new_status: str):
        gr.status = new_status
        gr.approved_by = approved_by
        gr.approved_at = datetime.now()

        if new_status == "completed":
            gr.completed_at = datetime.now()

    async def mark_all_receipts_completed(self, all_related_grs: List):
        now = datetime.now()

        for gr in all_related_grs:
            if gr.status in ['approved', 'has_issue']:
                gr.status = "completed"
                gr.completed_at = now

    async def handle_completed_case(self, gr, po, all_related_grs: List, approved_by: str, session: AsyncSession,
                                     status_info: Dict) -> Dict:
        await self.mark_all_receipts_completed(all_related_grs)

        stock_updates = await stock_update_service.update_stock_for_completed_receipts(
            all_related_grs=all_related_grs,
            warehouse_id=str(gr.warehouse_id),
            approved_by=approved_by,
            session=session
        )

        cache_service.delete(f"stock:warehouse:{str(gr.warehouse_id)}:summary")
        cache_service.delete_pattern(f"stock:low_stock:warehouse:{str(gr.warehouse_id)}:*")
        cache_service.delete_pattern(f"stock:warehouse:{str(gr.warehouse_id)}:product:*")
        cache_service.delete(f"stock:warehouse:{str(gr.warehouse_id)}:filters")

        logger.info(f"Invalidated stock caches for warehouse {gr.warehouse_id} after goods receipt approval")

        po.status = "completed"
        po.updated_at = datetime.now()

        return {
            "message": "Duyệt phiếu thành công. Đơn hàng đã nhận đủ hàng, tất cả phiếu đã hoàn tất và tồn kho đã được cập nhật",
            "data": {
                "id": str(gr.id),
                "receipt_number": gr.receipt_number,
                "status_change": {
                    "old_status": "pending",
                    "new_status": gr.status
                },
                "approval_info": {
                    "approved_by": str(gr.approved_by),
                    "approved_at": gr.approved_at.isoformat(),
                    "completed_at": gr.completed_at.isoformat() if gr.completed_at else None
                },
                "purchase_order": {
                    "id": str(po.id),
                    "po_number": po.po_number,
                    "status": po.status
                },
                "completion_status": {
                    "all_completed": True,
                    "stock_updated": True,
                    "total_variants_updated": len(stock_updates)
                },
                "stock_updates": stock_updates,
                "comparison_details": status_info['comparison_details']
            }
        }

    async def handle_partial_case(self, gr, po, po_details_map: Dict[str, any], session: AsyncSession,
                                  status_info: Dict) -> Dict:

        for detail in gr.receipt_details:
            po_detail = po_details_map.get(str(detail.po_detail_id))

            if po_detail:
                if po_detail.received_quantity is None:
                    po_detail.received_quantity = 0

                po_detail.received_quantity += detail.accepted_quantity

        po.status = "partial_received"
        po.updated_at = datetime.now()

        incomplete_items = [
            d for d in status_info['comparison_details']
            if not d['is_complete']
        ]

        return {
            "message": "Duyệt phiếu thành công. Phiếu có vấn đề do chưa nhận đủ hàng, cần tạo phiếu nhập bổ sung",
            "data": {
                "id": str(gr.id),
                "receipt_number": gr.receipt_number,
                "status_change": {
                    "old_status": "pending",
                    "new_status": gr.status
                },
                "approval_info": {
                    "approved_by": str(gr.approved_by),
                    "approved_at": gr.approved_at.isoformat(),
                    "completed_at": None
                },
                "purchase_order": {
                    "id": str(po.id),
                    "po_number": po.po_number,
                    "status": po.status
                },
                "completion_status": {
                    "all_completed": False,
                    "stock_updated": False,
                    "incomplete_items_count": len(incomplete_items),
                    "note": "Tồn kho chưa được cập nhật. Cần tạo phiếu nhập hàng bổ sung cho hàng còn thiếu."
                },
                "incomplete_items": incomplete_items,
                "comparison_details": status_info['comparison_details']
            }
        }