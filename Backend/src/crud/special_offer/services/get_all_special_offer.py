...
from src.schemas.special_offer import SpecialOfferFilterModel
...
class GetAllSpecialOfferService:
    async def get_all_special_offer(self, session: AsyncSession, filter_data: SpecialOfferFilterModel, skip: int = 0, limit: int = 10):
        ...
        special_offers, total = await special_offer_repository.get_all_special_offer(session=session, where_conditions=conditions, skip=skip, limit=limit)
    ...
    async def get_all_special_offer_customer(self, user_id: str, session: AsyncSession, search: Optional[str], skip: int = 0, limit: int = 10):
        ...
        special_offers, total = await special_offer_repository.get_all_special_offer(session=session, where_conditions=condition_offers, skip=skip, limit=limit)
